define(
    ...
    function ($) {
        ...
        return {
            isIbanValidForCountry: function (value, country) {
                if (ibanList[country] !== 'undefined') {
                    const ibanPattern = new RegExp(ibanList[country]);
                    return ibanPattern.test(value);
                }

                return true;
            },
            isSwiftValidForCountry: function (swiftCodeValue, ibanValue) {
                if (ibanValue !== "" && ibanValue.length > 1 && swiftCodeValue !== "") {
                    const firstLetter = ibanValue.substring(0, 1);
                    const secondLetter = ibanValue.substring(1, 2);
                    const swiftPattern = "^[a-zA-Z0-9]{4}[" + firstLetter + "]{1}[" + secondLetter + "]{1}[a-zA-Z0-9]{2}$|^[a-zA-Z0-9]{4}[" + firstLetter + "]{1}[" + secondLetter + "]{1}[a-zA-Z0-9]{5}$";
                    const swiftRegExp = new RegExp(swiftPattern);
                    return swiftRegExp.test(swiftCodeValue);
                }
